...
/datum/job/cyborg/get_description_blurb()
	return "Вы - корабельная синтетическая единица. Вы можете иметь личность, либо быть просто продвинутым набором программ - это определяет из чего сделанно устройство: \
	Киборги сделаны из мозга человека, Роботы - позитронного (искусственного) мозга, Дроны - простой печатной платы, не способной к владению личности. \
	У Вас есть доступ ко всем системам вокруг Вас, и конечности, чтобы передвигаться по объекту. \
	Не забывайте: Вы обязаны следовать своим законам, и только после этого приказам мастер-ИИ, к которому Вы привязаны. \
	Не наоборот. И только законам, если Вы не привязаны к ИИ. Чините поломки, спасайте людей и не открывайте двери куда не нужно - это вредит персоналу."
...
/datum/job/ai/get_description_blurb()
	return "Вы - Искусственный Интелект. Ваша обязанность - подчинятся установленным законам и помогать персоналу корабля в их работе. \
...
/datum/job/assistant/get_description_blurb()
	...
	Хоть корабль и не является флагманом в своём проекте, он заслужил репутацию надёжного объекта. Вы подчиняетесь Исполнительному Офицеру. \
