/obj/item/toy/plushie/skrell
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "blue plush skrell"
	desc = "A soft plush toy is so nice to hug it, On a little piece of paper that attached on a back of the toy is the message: PLEASE DON'T USE AS A SEX TOY."
	icon_state = "axicplushie"

/obj/item/toy/plushie/skrell/orange
	name = "Orange plush skrell"
	desc = "A soft plush toy is so nice to hug it, On a little piece of paper that attached on a back of the toy is the message: PLEASE DON'T USE AS A SEX TOY."
	icon_state = "qillplushie"

/obj/item/toy/plushie/skrell/red
	name = "Red plush skrell"
	desc = "A soft plush toy is so nice to hug it, On a little piece of paper that attached on a back of the toy is the message: PLEASE DON'T USE AS A SEX TOY."
	icon_state = "xanaplushie"

/obj/item/toy/plushie/fox
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Plush red fox"
	desc = "Cute and soft plush fox, it's so nice to hug it. After all, it is fluffy like a real fox."
	icon_state = "redfox"

/obj/item/toy/plushie/fox/blackfox
	name = "Plush black fox"
	desc = "Cute and soft plush fox, it's so nice to hug it. After all, it is fluffy like a real fox."
	icon_state = "blackfox"

/obj/item/toy/plushie/fox/orange
	name = "Plush orange fox"
	desc = "Cute and soft plush fox, it's so nice to hug it. After all, it is fluffy like a real fox."
	icon_state = "orangefox"


//Oh shit, here we fucking gooooooooooo
/* LC plushes */
// The good guys
/obj/item/toy/plushie/ayin
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "ayin plushie"
	desc = "A plushie depicting a researcher that did <b>nothing wrong</b>." // Fight me
	icon_state = "ayin"
	gender = MALE

/obj/item/toy/plushie/carmen
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Carmen plushie"
	desc = "A plushie depicting an ambitious and altruistic researcher."
	icon_state = "carmen"
	gender = FEMALE

// Sephirots
/obj/item/toy/plushie/malkuth
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Elijah plushie"
	desc = "A plushie depicting famous Shell Unit. Her name is Elijah. What a diligent worker."
	icon_state = "malkuth"
	gender = FEMALE

/obj/item/toy/plushie/yesod
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Gabriel plushie"
	desc = "A plushie depicting a Shell Research Unit in a turtleneck."
	icon_state = "yesod"
	gender = MALE

/obj/item/toy/plushie/netzach
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Giovani plushie"
	desc = "A plushie depicting a Shell Unit that likes alcohol a bit too much."
	icon_state = "netzach"
	gender = MALE

/obj/item/toy/plushie/hod
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Michel plushie"
	desc = "A plushie depicting a Shell Unit who hopes to make everything right."
	icon_state = "hod"
	gender = FEMALE

/obj/item/toy/plushie/lisa
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Lisa plushie"
	desc = "A plushie depicting a Shell Unit with high expectations."
	icon_state = "lisa"
	gender = FEMALE

/obj/item/toy/plushie/enoch
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Enoch plushie"
	desc = "A plushie depicting an optimistic Shell Unit with kind heart."
	icon_state = "enoch"
	gender = MALE

/obj/item/toy/plushie/chesed
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Daniel plushie"
	desc = "A plushie depicting a sleepy Shell Unit with a mug of coffee in their hand."
	icon_state = "chesed"
	gender = MALE

/obj/item/toy/plushie/gebura
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Kali plushie"
	desc = "A plushie depicting very strong and brave Shell Unit."
	icon_state = "gebura"
	gender = FEMALE

/obj/item/toy/plushie/hokma
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Benjamin plushie"
	desc = "A plushie depicting a wise Shell Unit with a fancy monocle. He knows much more than you."
	icon_state = "hokma"
	gender = MALE

/obj/item/toy/plushie/binah
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Garion plushie"
	desc = "A plushie depicting a sadistic Shell Unit who lacks any emotions."
	icon_state = "binah"
	gender = FEMALE

/obj/item/toy/plushie/angela
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Angela plushie"
	desc = "A plushie depicting The Supreme AI."
	icon_state = "angela"
	gender = FEMALE

	//Limbus Sinners
/obj/item/toy/plushie/yisang
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Yi sang plushie"
	desc = "A plushie depicting a ruminating sinner."
	icon_state = "yisang"
	gender = MALE

/obj/item/toy/plushie/faust
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Faust plushie"
	desc = "A plushie depicting an insufferable sinner."
	icon_state = "faust"
	gender = FEMALE

/obj/item/toy/plushie/don
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Don Quixote plushie"
	desc = "A plushie depicting a heroic sinner."
	icon_state = "don"
	gender = FEMALE

/obj/item/toy/plushie/ryoshu
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Ryoshu plushie"
	desc = "A plushie depicting a artistic sinner."
	icon_state = "ryoshu"
	gender = FEMALE

/obj/item/toy/plushie/meursault
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Meursault plushie"
	desc = "A plushie depicting a neutral sinner."
	icon_state = "meursault"
	gender = MALE

/obj/item/toy/plushie/honglu
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Hong Lu plushie"
	desc = "A plushie depicting a sheltered sinner."
	icon_state = "honglu"
	gender = MALE

/obj/item/toy/plushie/heathcliff
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Heathcliff plushie"
	desc = "A plushie depicting a brash sinner."
	icon_state = "heathcliff"
	gender = MALE

/obj/item/toy/plushie/ishmael
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Ishmael plushie"
	desc = "A plushie depicting a reliable sinner."
	icon_state = "ishmael"
	gender = FEMALE

/obj/item/toy/plushie/rodion
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Rodion plushie"
	desc = "A plushie depicting a backstreets born sinner."
	icon_state = "rodion"
	gender = FEMALE

/obj/item/toy/plushie/sinclair
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Sinclair plushie"
	desc = "A plushie depicting a insecure sinner."
	icon_state = "sinclair"
	gender = MALE

/obj/item/toy/plushie/dante
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Dante plushie"
	desc = "A plushie depicting a clock headed manager."
	icon_state = "dante"
	gender = MALE

/obj/item/toy/plushie/outis
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Outis plushie"
	desc = "A plushie depicting a strategic sinner."
	icon_state = "outis"
	gender = FEMALE

/obj/item/toy/plushie/gregor
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "bug guy plushie"
	desc = "A plushie depicting a genetically altered sinner."
	icon_state = "gregor"
	gender = MALE

// Misc LC stuff
/obj/item/toy/plushie/pierre
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Pierre plushie"
	desc = "A plushie depicting a friendly cook."
	icon_state = "pierre"
	gender = FEMALE

/obj/item/toy/plushie/myo
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Myo plushie"
	desc = "A plushie depicting a mercenary captain."
	icon_state = "myo"
	gender = FEMALE

/obj/item/toy/plushie/rabbit
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "rabbit plushie"
	desc = "A plushie depicting a Tau Cetian mercenary."
	icon_state = "rabbit"

/obj/item/toy/plushie/yuri
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "yuri plushie"
	desc = "A plushie depicting an The Cuchulain Foundation employee."
	icon_state = "yuri"
	gender = FEMALE

// /obj/item/toy/plushie/attackby(obj/item/I, mob/living/user, params)
// 	. = ..()
// 	if(istype(I, /obj/item/reagent_containers/food/snacks/grown/goldapple))
// 		if(do_after(user, 2 SECONDS, target = user))
// 			user.visible_message("<span class='notice'>[src] is violently absorbed by the [I]!</span>")
// 			qdel(src)
// 			return
// 		to_chat(user, "<span class='notice'>You feel as if you prevented something weird and terrible from happening again.</span>")

/obj/item/toy/plushie/blank
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "plushie blank"
	desc = "A humanoid plush that had been freshly made or stripped down to its cloth. Despite its lack of identity, the mere aknowelegement of this plushie makes it unique."
	icon_state = "blank"


// Abnormalities
/obj/item/toy/plushie/qoh
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "queen of hatred plushie"
	desc = "A plushie depicting a magical girl whose goal is fighting all evil in the universe!"
	icon_state = "qoh"
	gender = FEMALE

/obj/item/toy/plushie/kog
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "king of greed plushie"
	desc = "A plushie depicting a magical girl whose desires got the best of her."
	icon_state = "kog"
	gender = FEMALE

/obj/item/toy/plushie/kod
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "knight of despair plushie"
	desc = "A plushie depicting a magical girl who abandoned those who needed her most."
	icon_state = "kod"
	gender = FEMALE

/obj/item/toy/plushie/sow
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "servant of wrath plushie"
	desc = "A plushie depicting a magical girl who was betrayed by someone they trusted dearly."
	icon_state = "sow"
	gender = FEMALE

/obj/item/toy/plushie/nihil
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "jester of nihil plushie"
	desc = "A plushie depicting a black and white jester, usually found alongside the magical girls."
	icon_state = "nihil"

/obj/item/toy/plushie/bigbird
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "big bird plushie"
	desc = "A plushie depicting a big bird with a lantern that burns forever. How does it even work..?"
	icon_state = "bigbird"

/obj/item/toy/plushie/mosb
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "mountain of smiling bodies plushie"
	desc = "A plushie depicting a mountain of corpses merged into one. Yuck!"
	icon_state = "mosb"

/obj/item/toy/plushie/big_bad_wolf
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "big and will be bad wolf plushie"
	desc = "A plushie depicting quite a not so bad and very much so marketable wolf plush."
	icon_state = "big_bad_wolf"

/obj/item/toy/plushie/melt
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "melting love plushie"
	desc = "A plushie depicting a slime girl, you think."
	icon_state = "melt"

/obj/item/toy/plushie/scorched
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "scorched girl plushie"
	desc = "A plushie depicting scorched girl."
	icon_state = "scorched"
	gender = FEMALE

// Others
/obj/item/toy/plushie/bongbong
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "bongbong plushie"
	desc = "A plushie depicting the The Cuchulain Foundation"
	icon_state = "bongbong"

//Spawners
/obj/random_multi/single_item/malkuth
	name = "Multi Point - Malkuth"
	id = "Malkuth"
	item_path = /obj/item/toy/plushie/malkuth

/obj/random_multi/single_item/netzach
	name = "Multi Point - Netzach"
	id = "Netzach"
	item_path = /obj/item/toy/plushie/netzach

/obj/random_multi/single_item/yesod
	name = "Multi Point - Yesod"
	id = "Yesod"
	item_path = /obj/item/toy/plushie/yesod

/obj/random_multi/single_item/hod
	name = "Multi Point - Hod"
	id = "Hod"
	item_path = /obj/item/toy/plushie/hod

/obj/random_multi/single_item/enoch
	name = "Multi Point - Enoch"
	id = "Enoch"
	item_path = /obj/item/toy/plushie/enoch

/obj/random_multi/single_item/lisa
	name = "Multi Point - Lisa"
	id = "Lisa"
	item_path = /obj/item/toy/plushie/lisa

/obj/random_multi/single_item/chesed
	name = "Multi Point - Chesed"
	id = "Chesed"
	item_path = /obj/item/toy/plushie/chesed

/obj/random_multi/single_item/gebura
	name = "Multi Point - Gebura"
	id = "Gebura"
	item_path = /obj/item/toy/plushie/gebura

/obj/random_multi/single_item/hokma
	name = "Multi Point - Hokma"
	id = "Hokma"
	item_path = /obj/item/toy/plushie/hokma

/obj/random_multi/single_item/binah
	name = "Multi Point - Binah"
	id = "Binah"
	item_path = /obj/item/toy/plushie/binah

/obj/random_multi/single_item/angela
	name = "Multi Point - Angela"
	id = "Angela"
	item_path = /obj/item/toy/plushie/angela

/obj/item/toy/plushie/fumo
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Plushie black witch"
	desc = "The black witch from the popular Yangui cartoon, well, one is pretty cute."
	icon_state = "fumoplushie_marisa"
	gender = FEMALE

/obj/item/toy/plushie/fumo/cirno
	icon = 'proxima/icons/obj/prx_toy.dmi'
	name = "Plushie winter fairy"
	desc = "Silly winter fairy from the popular Yangui cartoon . You have to squeeze her cheeks."
	icon_state = "fumoplushie_cirno"
	gender = FEMALE

/obj/random_multi/single_item/witch
	name = "Multi Point - witch"
	id = "witch"
	item_path = /obj/item/toy/plushie/fumo

/obj/random_multi/single_item/cirno
	name = "Multi Point - fairy"
	id = "fairy"
	item_path = /obj/item/toy/plushie/fumo/cirno
